...
run_auditd_hardening() {
  ...
  if [[ -f "$rules" ]] && cmp -s "$tmp" "$rules" && service_is_active auditd; then
    log_success "Auditd hardening already configured and service is active"
    report_add_already_configured "$rules"
    rm -f "$tmp"
    return 0
  fi
  ...
  if [[ "${DRY_RUN:-false}" != "true" ]]; then
    report_mark_changed "Auditd rules loaded"
